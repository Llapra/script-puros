...
def interpretar_nubosidad(df_pred):
    """
    Interpreta los valores de Radiación Neta para estimar la cobertura nubosa.
    """
    print("\n" + "="*40)
    ...
    for tiempo, fila in df_pred.iterrows():
        rad = fila['NETRAD']
        hora = tiempo.hour
        es_dia = 7 <= hora <= 19  # Definición simple de día/noche
        
        estado = "Desconocido"
        icono = "❓"
        
        if es_dia:
            # Umbrales diurnos (Radiación Positiva)
            if rad > 400:
                estado = "DESPEJADO / SOLEADO"
                icono = "☀️"
            elif rad > 200:
                estado = "PARCIALMENTE NUBLADO"
                icono = "⛅"
            else:
                estado = "NUBLADO / CUBIERTO"
                icono = "☁️"
        else:
            # Umbrales nocturnos (Radiación Negativa)
            # Cielo despejado pierde mucho calor -> Muy negativo
            # Cielo nublado rebota calor -> Cerca de cero
            if rad < -40:
                estado = "DESPEJADO (Frío)"
                icono = "🌙"
            else:
                estado = "NUBLADO (Templado)"
                icono = "☁️"
...
if __name__ == "__main__":
    ...
    print("Sistema listo. Esperando fecha...")
    ...
    while True:
        try:
            ...
            if X_vis is None:
                print("Error: Datos insuficientes para esta fecha.")
                continue
